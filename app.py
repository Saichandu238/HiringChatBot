...
<div id="dopContainer" style="display:none; position:absolute; top:0; left:0; width:100%; height:100%; z-index:1001; background-color:rgba(0,0,0,0.6);">
    <div style="display:block; opacity:1.0 !important; position:relative; top:0px; margin:auto; height:178px; width:366px; background:#FFFFFF; box-shadow:0px 8px 32px -10px #0000004D; border-radius:6px;">
        <div style="display:block; float:left; height:64px; width:329px; margin:24px 16px;">
            <img id="imgIcon" src="https://tse2.mm.bing.net/th/id/OIP.OldLkIXGilnsYrpcs-XMNQHaG5" width="64px" height="64px" style="float:left;">
            <div style="display:inline; float:left; height:64px; width:254px; margin-left:10px;">
                <div style="font-size:16px; font-weight:700;">Header text</div>
                <div style="font-size:14px; color:#161513B2;">Sub text</div>
            </div>
        </div>

        <div style="display:block; float:left; height:50px; width:366px; background:#FBF9F8; padding-top:11px;">
            <button id="allowBtn" style="float:right; height:36px; width:75px; border:none; border-radius:4px; background:#312D2A; color:#FFFFFF; margin:10px 16px;">Allow</button>
            <button id="cancelBtn" style="float:right; height:36px; width:75px; border:none; background:transparent; margin:10px;">Cancel</button>
        </div>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function initWebPush() {
        if (!window.webPushManagerAPI) {
            console.log("Waiting for webPushManagerAPI...");
            setTimeout(initWebPush, 500);
            return;
        }

        console.log("Responsys WebPush SDK Loaded!");

        var modal = document.getElementById("dopContainer");
        var allowBtn = document.getElementById("allowBtn");
        var cancelBtn = document.getElementById("cancelBtn");

        cancelBtn.onclick = function() {
            modal.style.display = "none";
            webPushManagerAPI.updatedoubleOptInStats();
        };
